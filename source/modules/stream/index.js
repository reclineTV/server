...
module.exports = app => {
	...
	*/
			...
			if(options.provider){
				...
				app.database.query(
					...
					(err, res) => {
						...
						loadStream(urlParts[0], urlParts[1], success, failed);
					}
				);
	...
	function loadStream(providerRef, mediaRef, success, failed){
		...
		return Promise.resolve(provider.stream(mediaRef))
			// Then transcode it:
